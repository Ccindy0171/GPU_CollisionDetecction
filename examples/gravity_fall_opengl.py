...
def main():
    ...
    # Configuration
    # ========================================================================
    NUM_OBJECTS = 500           # Number of balls
    WORLD_BOUNDS = ((-20, 0, -20), (20, 40, 20))  # World boundaries
    CELL_SIZE = 2.0             # Grid cell size
    NUM_FRAMES = 600            # Total frames (10 seconds @ 60fps)
    ...
    def render():
        ...
        # Check for overlaps (for debugging)
        num_overlaps = 0
        if frame_count % 60 == 0:  # Check every second
            for i in range(min(NUM_OBJECTS, 100)):  # Sample for performance
                for j in range(i+1, min(NUM_OBJECTS, 100)):
        ...
        if frame_count % report_interval == 0:
            ...
            print(f"Frame {frame_count:4d}/{NUM_FRAMES}: "
                  f"Collisions={stats['num_collisions']:4d}, "
                  f"SimFPS={1000.0/stats['total_time']:4.0f}, "
                  f"RenderFPS={render_fps:4.1f}")
